...
import com.depakto.classes.help.commands.CChannelDetails;
...
import com.depakto.classes.help.commands.CClientGetNickname;
...
public class TSApi {
    ...
    public Channel getChannelDetails(int channelId) {
        CChannelDetails info = new CChannelDetails(channelId);
        if (this.query.doCommand(info)) {
            return new Channel(info.getFirstResponse().getMap());
        }
        return null;
    }
    ...
    public ClientInfo getClientByUId(String clientUId) {
        ...
        if (this.query.doCommand(get)) {
            return getClientInfo(StringUtil.getInt((String) get.getFirstResponse().getMap().get("clid")), true);
        }
        return null;
    }

    public ClientInfo getClientInfo(int clientId, boolean trybWywolania) {
        CClientInfo info = new CClientInfo(clientId, trybWywolania);
        if (this.query.doCommand(info)) {
    ...
    public VirtualServerInfoClient getServerInfoClient() {
        CServerInfoClient info = new CServerInfoClient();
        if (this.query.doCommand(info)) {
            return new VirtualServerInfoClient(info.getFirstResponse().getMap());
        }
        return null;
    }

    public VirtualServerIP getServerIP() {
        CServerIP info = new CServerIP();
        if (this.query.doCommand(info)) {
            return new VirtualServerIP(info.getFirstResponse().getMap());
        }
        return null;
    }
    ...
    public boolean setAwayMessage(String text) {
        HashMap<ClientProperty, String> options = new HashMap();
        options.put(ClientProperty.CLIENT_AWAY_MESSAGE, text);
        return updateClient(options);
    }
    ...
    public String getNicknameFromUid(String uid) {
        CClientGetNickname dane = new CClientGetNickname(uid);
        if (this.query.doCommand(dane)) {
            return dane.getFirstResponse().toString();
        }
